...
class Registration{
    ...
    public function __construct(){
        ...
        add_action( 'wp_ajax_XWOO_registration', array( $this, 'registration_save_action' ) );
        add_action( 'wp_ajax_nopriv_XWOO_registration', array( $this, 'registration_save_action' ) );
    }
            ...
            <div class="wpneo-user-registration-wrap">
                ...
                <?php echo wp_nonce_field( 'XWOO_form_action', 'XWOO_form_action_field', true, false ); ?>
                    ...
                    $regisration_data = array(
                        ...
                        array(
                            'id'            => 'password',
                            'label'         => __('Password *', 'xwoo'),
                            'type'          => 'password',
                            'placeholder'   => __('Enter Password', 'xwoo'),
                            'class'         => 'required',
                            'warpclass'     => '',
                            'autocomplete'  => 'off',
                        ),
    ...
    public function registration_save_action() {
        ...
        //Add some option
        do_action( 'XWOO_before_user_registration_action' );
    ...
    public function complete_registration( $username, $password, $email, $website, $first_name, $last_name, $nickname, $bio ) {
        ...
        if ( count($reg_errors->get_error_messages()) < 1 ) {
            ...
            if ( !is_wp_error( $user_id ) ) {
                ...
                do_action( 'XWOO_after_user_registration', $user_id );
    
                $saved_redirect_uri = get_option('XWOO_user_reg_success_redirect_uri');
                $redirect = $saved_redirect_uri ? $saved_redirect_uri : esc_url( home_url( '/' ) );
                die(json_encode(array('success'=> 1, 'message' => __('Registration complete.', 'xwoo'), 'redirect' => $redirect )));
            } else {
                $errors = '';
                if ( is_wp_error( $reg_errors ) ) {
                    foreach ( $reg_errors->get_error_messages() as $error ) {
                        $errors .= '<strong>'.__('ERROR','xwoo').'</strong>:'.$error.'<br />';
                    }
                }
    ...
    public function registration_validation( $username, $password, $email, $website, $first_name, $last_name, $nickname, $bio ) {
        ...
        if ( empty( $username ) || empty( $password ) || empty( $email ) ) {
            $reg_errors->add('field', __('Required form field is missing','xwoo'));
        }
    
        if ( strlen( $username ) < 4 ) {
            $reg_errors->add('username_length', __('Username too short. At least 4 characters is required','xwoo'));
        }
    
        if ( username_exists( $username ) )
            $reg_errors->add('user_name', __('Sorry, that username already exists!','xwoo'));
    
        if ( !validate_username( $username ) ) {
            $reg_errors->add('username_invalid', __('Sorry, the username you entered is not valid','xwoo'));
        }
    
        if ( strlen( $password ) < 6 ) {
            $reg_errors->add('password', __('Password length must be greater than 6','xwoo'));
        }
    
        if ( !is_email( $email ) ) {
            $reg_errors->add('email_invalid', __('Email is not valid','xwoo'));
        }
    
        if ( email_exists( $email ) ) {
            $reg_errors->add('email', __('Email Already in use','xwoo'));
        }
    
        if ( !empty( $website ) ) {
            if ( !filter_var($website, FILTER_VALIDATE_URL) ) {
                $reg_errors->add('website', __('Website is not a valid URL','xwoo'));
            }
        }
    }
}
