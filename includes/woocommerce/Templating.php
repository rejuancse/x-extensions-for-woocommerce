...
class Templating {
    ...
    public function __construct() {
        ...
        //Set Theme
        $is_new_theme = true;
        if( !file_exists( get_stylesheet_directory()."/XWOOtemplate/{$this->_vendor}/{$this->_theme}/style.css" ) ){
            $is_new_theme = false;
        }
    ...
    public function theme_noticed_callback(){
        ...
        $themes_dir = $this->XWOO_select_themes_dir();
        $html = "";
                ...
                $html .='<td>';
                    $html .= '<select name="wpneo_cf_selected_theme">';
                        if (count($themes_dir) > 0){
                            ...
                            $html .= '<option value="">'.__('You have no valid theme', 'xwoo').'</option>';
                        }
                    $html .= '</select>';
                $html .= '</td>';
            $html .='</tr>';
     ...
     */
    public function XWOO_select_themes_dir(){
        $theme_dirs = array_filter(glob(XWOO_DIR_PATH.'XWOOtemplate/woocommerce/*'), 'is_dir');
        $get_dir = array();
    ...
    public function load_theme_css_callback(){
        ...
        if ($is_valid_theme) {
            if (file_exists($this->_theme_in_themes_path.'style.css')){
                wp_enqueue_style('XWOO_style', $this->_selected_theme_uri.'style.css',array(), XWOO_VERSION);
            }else{
                wp_enqueue_style('XWOO_style', $this->_selected_theme_uri.'style.css',array(), XWOO_VERSION);
            }
        }else{
