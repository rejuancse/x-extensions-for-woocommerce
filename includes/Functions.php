...
class Functions {
    ...
    public function get_campaign_update_status(){
        ...
        $html .= '<h3>';
        $html .= apply_filters( 'XWOO_campaign_update_title', __( $post->post_title.'\'s Update','xwoo' ) );
        $html .= '</h3>';
    ...
    public function campaign_start_countdown($post_id = 0){
        ...
        <script type="text/javascript">
            ...
            let XWOOCountDownDate = "<?php echo $_nf_duration_start; ?>".split("-");
            XWOOCountDownDate = new Date( XWOOCountDownDate[1]+"/"+XWOOCountDownDate[0]+"/"+XWOOCountDownDate[2] ).getTime();

            // Update the count down every 1 second
            let XWOOIntervalE = setInterval(function() {
                // Get toXWOODays date and time
                const dateDiff = XWOOCountDownDate - new Date().getTime();

                // Time calculations
                let XWOODays = Math.floor(dateDiff / 86400000 );
                let XWOOHours = Math.floor((dateDiff % 86400000 ) / 3600000 );
                let XWOOMinutes = Math.floor((dateDiff % 3600000 ) / 60000 );
                let XWOOSeconds = Math.floor((dateDiff % 60000 ) / 1000 );

                // Display the result in the element with id="XWOO-campaign-countdown"
                document.getElementById("XWOO-campaign-countdown").innerHTML = '<span>'+XWOODays+'</span>' + "d " + "<span>" + XWOOHours + "h </span> <span> " + XWOOMinutes + "m </span> <span> " + XWOOSeconds + "s </span> ";

                // If the count down is finished, write some text
                if ( dateDiff < 0 ) {
                    clearInterval(XWOOIntervalE);
                    document.getElementById("XWOO-campaign-countdown").innerHTML = "";
                }
            }, 1000);
        </script>
