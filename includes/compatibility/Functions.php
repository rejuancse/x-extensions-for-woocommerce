...
function wp_xwoo_get_author_name(){
...
function wp_xwoo_get_campaigns_location(){
    return XWOO_function()->campaign_location();
}

function wp_xwoo_get_total_fund_raised_by_campaign($campaign_id = 0){
    return XWOO_function()->fund_raised($campaign_id);
}

function wp_xwoo_get_total_goal_by_campaign($campaign_id){
    return XWOO_function()->total_goal($campaign_id);
}

function wp_xwoo_price($price, $args = array()){
    return XWOO_function()->price( $price, $args = array() );
}

function wp_loved_campaign_count($user_id = 0){
...
function wp_xwoo_wc_login_form(){
    return XWOO_function()->login_form();
}

function wp_xwoo_author_all_campaigns($author_id = 0){
    return XWOO_function()->author_campaigns( $author_id );
}

function wp_xwoo_add_http($url){
    return XWOO_function()->url($url);
}

function wp_xwoo_embeded_video($url){
    return XWOO_function()->get_embeded_video( $url );
}

function wp_xwoo_campaign_listing_by_author_url($user_login){
    return XWOO_function()->get_author_url( $user_login );
}

function wp_xwoo_load_template($template = '404'){
    return XWOO_function()->template($template);
}

function wp_xwoo_pagination($page_numb, $max_page) {
    return XWOO_function()->get_pagination($page_numb, $max_page);
}

function wp_wc_version_check($version = '3.0') {
    return XWOO_function()->wc_version($version = '3.0');
}

function wp_xwoo_campaign_single_love_this() {
