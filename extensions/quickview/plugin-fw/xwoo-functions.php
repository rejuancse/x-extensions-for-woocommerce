 ...
 * Functions.
 *
 * @package XWOO\PluginFramework
 */

defined( 'ABSPATH' ) || exit; // Exit if accessed directly.

if ( ! function_exists( 'xwoo_plugin_locate_template' ) ) {
	/**
	 ...
	 * @since 2.0.0
	 */
	function xwoo_plugin_locate_template( $plugin_basename, $path, $var = null ) {
...
if ( ! function_exists( 'xwoo_plugin_get_template' ) ) {
	/**
	 ...
	 * @since 2.0.0
	 */
	function xwoo_plugin_get_template( $plugin_basename, $path, $var = null, $return = false ) {

		$located = xwoo_plugin_locate_template( $plugin_basename, $path, $var );
...
if ( ! function_exists( 'xwoo_plugin_content' ) ) {
	/**
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_plugin_content( $what = 'content', $limit = 25, $more_text = '', $split = '[...]', $in_paragraph = true ) {
		if ( 'content' === $what ) {
		...
		if ( ! empty( $more_text ) && ! isset( $more_link ) ) {
			$more_link = strpos( $more_text, '<a class="btn"' ) ? $more_text : '<a class="read-more' . apply_filters( 'xwoo_simple_read_more_classes', ' ' ) . '" href="' . get_permalink() . '">' . $more_text . '</a>';
			$split     = '';
...
if ( ! function_exists( 'xwoo_plugin_string' ) ) {
	/**
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_plugin_string( $before = '', $string = '', $after = '', $echo = true ) {
		$html = '';
...
if ( ! function_exists( 'xwoo_plugin_decode_title' ) ) {
	/**
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_plugin_decode_title( $title ) {
		$replaces = apply_filters( 'xwoo_title_special_characters', array() );

		return preg_replace( array_keys( $replaces ), array_values( $replaces ), $title );
	}
}

if ( ! function_exists( 'xwoo_plugin_get_attachment_id' ) ) {
	 ...
	 * @since 2.0.0
	 */
	function xwoo_plugin_get_attachment_id( $url ) {
...
if ( ! function_exists( 'xwoo_enqueue_script' ) ) {
	/**
	 * Enqueues script.
	 ...
	 *
	 * @author     Simone D'Amico <simone.damico@xwooemes.com>
	 * @deprecated 3.5
	 */
	function xwoo_enqueue_script( $handle, $src, $deps = array(), $ver = false, $in_footer = true ) {
...
if ( ! function_exists( 'xwoo_enqueue_style' ) ) {
	/**
	 * Enqueue Styles.
	 * IMPORTANT: used only in themes, since it needs the YIT_Asset class.
	 *
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_enqueue_style( $handle, $src, $deps = array(), $ver = false, $media = 'all' ) {
...
if ( ! function_exists( 'xwoo_get_post_meta' ) ) {
	/**
	 ...
	 * @since    2.0.0
	 */
	function xwoo_get_post_meta( $id, $meta ) {
		if ( ! strpos( $meta, '[' ) ) {
...
if ( ! function_exists( 'xwoo_string' ) ) {
	/**
	 * Simple echo a string, with a before and after string, only if the main string is not empty.
	 *
	 * @param string $before What there is before the main string.
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_string( $before = '', $string = '', $after = '', $echo = true ) {
		$html = '';
...
if ( ! function_exists( 'xwoo_pagination' ) ) {
	/**
	 * Print pagination
	 *
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_pagination( $pages = '', $range = 10 ) {
		...
		if ( 1 !== $pages ) {
			...
			if ( $paged > 2 ) {
				$html .= sprintf( '<a class="%s" href="%s">&laquo;</a>', 'xwoo_pagination_first', get_pagenum_link( 1 ) );
			}
			if ( $paged > 1 ) {
				$html .= sprintf( '<a class="%s" href="%s">&lsaquo;</a>', 'xwoo_pagination_previous', get_pagenum_link( $paged - 1 ) );
			}
			...
			if ( $paged < $pages ) {
				$html .= sprintf( '<a class="%s" href="%s">&rsaquo;</a>', 'xwoo_pagination_next', esc_url( get_pagenum_link( $paged + 1 ) ) );
			}
			if ( $paged < $pages - 1 ) {
				$html .= sprintf( '<a class="%s" href="%s">&raquo;</a>', 'xwoo_pagination_last', esc_url( get_pagenum_link( $pages ) ) );
			}

			$html .= "</div>\n";
		}

		echo apply_filters( 'xwoo_pagination_html', $html ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}

if ( ! function_exists( 'xwoo_registered_sidebars' ) ) {
	/**
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_registered_sidebars() {
...
if ( ! function_exists( 'xwoo_layout_option' ) ) {
	/**
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_layout_option( $key, $id = false, $type = 'post', $model = 'post_type' ) {
		$option = '';
...
if ( ! function_exists( 'xwoo_curPageURL' ) ) {
	/**
	 * Retrieve the current complete url
	 *
	 * @since 1.0
	 */
	function xwoo_curPageURL() { 
		$page_url = 'http';
...
if ( ! function_exists( 'xwoo_get_excluded_categories' ) ) {
	/**
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_get_excluded_categories( $k = 1 ) {
		global $post;
		...
		$cats = function_exists( 'xwoo_get_option' ) ? xwoo_get_option( 'blog-excluded-cats' ) : '';
...
if ( ! function_exists( 'xwoo_add_extra_theme_headers' ) ) {
	add_filter( 'extra_theme_headers', 'xwoo_add_extra_theme_headers' );
	/**
	 ...
	 */
	function xwoo_add_extra_theme_headers( $headers ) {
...
if ( ! function_exists( 'xwoo_check_plugin_support' ) ) {
	/**
	 * Check the framework core version
	 *
	 * @return bool
	 ...
	 * @author Andrea Grillo <andrea.grillo@xwooemes.com>
	 */
	function xwoo_check_plugin_support() {
...
if ( ! function_exists( 'xwoo_ie_version' ) ) {
	/**
	 * Retrieve IE version.
	 *
	 * @return int|float
	 * @since  1.0.0
	 * @author Andrea Grillo <andrea.grillo@xwooemes.com>, Andrea Frascaspata<andrea.frascaspata@xwooemes.com>
	 */
	function xwoo_ie_version() {
...
if ( ! function_exists( 'xwoo_avoid_duplicate' ) ) {
	/**
	 ...
	 * @return mixed
	 * @since  2.0.0
	 * @author Antonino Scarfì <antonino.scarfi@xwooemes.com>
	 */
	function xwoo_avoid_duplicate( $value, $array, $check = 'value' ) {
		$match = array();
		...
		if ( ( 'value' === $check && ! in_array( $value, $array, true ) ) || ( 'key' === $check && ! isset( $array[ $value ] ) ) ) {
			...
			return xwoo_avoid_duplicate( $value . '-' . $i, $array, $check );
		}
	}
}

if ( ! function_exists( 'xwoo_title_special_characters' ) ) {
	/**
	 * The chars used in xwoo_decode_title() and xwoo_encode_title()
	 * E.G.
	 * string: This is [my title] with | a new line
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_title_special_characters( $chars ) {
		return array_merge(
			$chars,
			array(
	...
	add_filter( 'xwoo_title_special_characters', 'xwoo_title_special_characters' );
}

if ( ! function_exists( 'xwoo_decode_title' ) ) {
	/**
	 * Change some special characters to put easily html into a string
	 * E.G.
	 * string: This is [my title] with | a new line
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_decode_title( $title ) {
		$replaces = apply_filters( 'xwoo_title_special_characters', array() );
...
if ( ! function_exists( 'xwoo_encode_title' ) ) {
	/**
	 * Change some special characters to put easily html into a string
	 * E.G.
	 * string: This is [my title] with | a new line
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_encode_title( $title ) {
		$replaces = apply_filters( 'xwoo_title_special_characters', array() );
...
if ( ! function_exists( 'xwoo_remove_chars_title' ) ) {
	/**
	 * Change some special characters to put easily html into a string
	 * E.G.
	 * string: This is [my title] with | a new line
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_remove_chars_title( $title ) {
		$replaces = apply_filters( 'xwoo_title_special_characters', array() );
...
if ( ! function_exists( 'is_shop_installed' ) ) {
	 ...
	 * @return bool
	 * @since  2.0.0
	 * @author Francesco Grasso <francesco.grasso@xwooemes.com
	 */
...
if ( ! function_exists( 'xwoo_load_js_file' ) ) {
	/**
	 ...
	 * @author Andrea Grillo <andrea.grillo@xwooemes.com>
	 */
	function xwoo_load_js_file( $filename ) {

		if ( ! ( ( defined( 'WP_DEBUG' ) && WP_DEBUG ) || ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) || isset( $_GET['xwoo_script_debug'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
...
if ( ! function_exists( 'xwoo_load_css_file' ) ) {
	/**
	 ...
	 */
	function xwoo_load_css_file( $filename ) {
...
if ( ! function_exists( 'xwoo_wpml_register_string' ) ) {
	/**
	 ...
	 *
	 * @since  2.0.0
	 * @author Andrea Frascaspata <andrea.frascaspata@xwooemes.com>
	 */
	function xwoo_wpml_register_string( $context, $name, $value ) {
		do_action( 'wpml_register_single_string', $context, $name, $value );
	}
}

if ( ! function_exists( 'xwoo_wpml_string_translate' ) ) {
	/**
	 ...
	 * @author Andrea Frascaspata <andrea.frascaspata@xwooemes.com>
	 */
	function xwoo_wpml_string_translate( $context, $name, $default_value ) {
		return apply_filters( 'wpml_translate_single_string', $default_value, $context, $name );
	}
}

if ( ! function_exists( 'xwoo_wpml_object_id' ) ) {
	/**
	 ...
	 * @since  2.0.0
	 * @author Antonio La Rocca <antonio.larocca@xwooemes.com>
	 */
	function xwoo_wpml_object_id( $element_id, $element_type = 'post', $return_original_if_missing = false, $language_code = null ) {
...
if ( ! function_exists( 'xwoo_get_formatted_price' ) ) {
	/**
	 ...
	 * @return string
	 */
	function xwoo_get_formatted_price( $price, $args = array() ) {
		$defaults = array(
			'ex_tax_label'       => false,
			'currency'           => '',
		...
		list ( $decimals, $decimal_separator, $thousand_separator, $price_format, $currency ) = xwoo_plugin_fw_extract( $args, 'decimals', 'decimal_separator', 'thousand_separator', 'price_format', 'currency' );
...
if ( ! function_exists( 'xwoo_get_terms' ) ) {
	/**
	 * Get terms.
	 *
	 * @param array $args Arguments.
...
if ( ! function_exists( 'xwoo_field_deps_data' ) ) {
	/**
	 ...
	 * @return string
	 */
	function xwoo_field_deps_data( $field ) {
		$deps_data = '';
...
if ( ! function_exists( 'xwoo_panel_field_deps_data' ) ) {
	/**
	 ...
	 * @return string
	 */
	function xwoo_panel_field_deps_data( $field, $panel ) {
		$deps_data = '';
		if ( isset( $field['deps'] ) && ( isset( $field['deps']['ids'] ) || isset( $field['deps']['id'] ) ) && isset( $field['deps']['values'] ) ) {
			...
			$deps_data = xwoo_field_deps_data( $field );
		}
...
if ( ! function_exists( 'xwoo_plugin_fw_get_field' ) ) {
	/**
	 * Retrieve a field.
	 *
	 * @param array $field          The field.
	 ...
	 * @return false|string
	 */
	function xwoo_plugin_fw_get_field( $field, $echo = false, $show_container = true ) {
		if ( empty( $field['type'] ) ) {
			return '';
		}
		...
		$field_template = xwoo_plugin_fw_get_field_template_path( $field );
		...
		if ( $field_template ) {
			...
			do_action( 'xwoo_plugin_fw_get_field_' . $field['type'] . '_before', $field );

			include $field_template;

			do_action( 'xwoo_plugin_fw_get_field_after', $field );
			do_action( 'xwoo_plugin_fw_get_field_' . $field['type'] . '_after', $field );
...
if ( ! function_exists( 'xwoo_plugin_fw_get_field_template_path' ) ) {
	/**
	 ...
	 * @return false|string
	 */
	function xwoo_plugin_fw_get_field_template_path( $field ) {
		if ( empty( $field['type'] ) ) {
			return false;
		}
		...
		$field_template = apply_filters( 'xwoo_plugin_fw_get_field_template_path', $field_template, $field );

		return file_exists( $field_template ) ? $field_template : false;
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_html_data_to_string' ) ) {
	/**
	 ...
	 * @return string
	 */
	function xwoo_plugin_fw_html_data_to_string( $data = array(), $echo = false ) {
		$html_data = '';
...
if ( ! function_exists( 'xwoo_plugin_fw_get_icon' ) ) {
	/**
	 * Retrieve an icon.
	 *
	 ...
	 * @return string
	 */
	function xwoo_plugin_fw_get_icon( $icon = '', $args = array() ) {
		return YIT_Icons()->get_icon( $icon, $args );
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_is_true' ) ) {
	/**
	 * Is something true?
	 *
	 ...
	 * @return bool
	 */
	function xwoo_plugin_fw_is_true( $value ) {
		return true === $value || 1 === $value || '1' === $value || 'yes' === $value || 'true' === $value;
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_enqueue_enhanced_select' ) ) {
	/**
	 * Enqueue the enhanced select style and script.
	 */
	function xwoo_plugin_fw_enqueue_enhanced_select() {
		wp_enqueue_script( 'xwoo-enhanced-select' );
		$select2_style_to_enqueue = function_exists( 'WC' ) ? 'woocommerce_admin_styles' : 'xwoo-select2-no-wc';
		wp_enqueue_style( $select2_style_to_enqueue );
	}
}

if ( ! function_exists( 'xwoo_add_select2_fields' ) ) {
	/**
	 * Add select 2.
	 *
	 * @param array $args The arguments.
	 */
	function xwoo_add_select2_fields( $args = array() ) {
		$default = array(
			'type'              => 'hidden',
			'class'             => '',
			'id'                => '',
...
if ( ! function_exists( 'xwoo_plugin_fw_get_version' ) ) {
	/**
	 * Retrieve the Plugin Framework version.
	 *
	 * @return string
	 */
	function xwoo_plugin_fw_get_version() {
...
if ( ! function_exists( 'xwoo_get_premium_support_url' ) ) {
	/**
	 ...
	 * @deprecated 3.5
	 */
	function xwoo_get_premium_support_url() {
		return 'https://xwooemes.com/my-account/support/dashboard/';
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_is_panel' ) ) {
	/**
	 * Is this a Plugin Framework panel?
	 *
	 * @return bool
	 */
	function xwoo_plugin_fw_is_panel() {
		$panel_screen_id = 'xwoo-plugins_page';
...
if ( ! function_exists( 'xwoo_plugin_fw_force_regenerate_plugin_update_transient' ) ) {
	/**
	 ...
	 * @author Andrea Grillo <andrea.grillo@xwooemes.com>
	 */
	function xwoo_plugin_fw_force_regenerate_plugin_update_transient() {
		delete_site_transient( 'update_plugins' );
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_is_gutenberg_enabled' ) ) {
	/**
	 * Is Gutenberg enabled?
	 *
	 * @return bool
	 */
	function xwoo_plugin_fw_is_gutenberg_enabled() {
		return function_exists( 'XWOO_Gutenberg' );
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_gutenberg_add_blocks' ) ) {
	/**
	 ...
	 * @author Andrea Grillo <andrea.grillo@xwooemes.com>
	 */
	function xwoo_plugin_fw_gutenberg_add_blocks( $blocks ) {
		$added = false;
		if ( xwoo_plugin_fw_is_gutenberg_enabled() ) {
			// Add blocks.
			$added = XWOO_Gutenberg()->add_blocks( $blocks );

			// Add blocks arguments.
			if ( $added ) {
				XWOO_Gutenberg()->set_block_args( $blocks );
			}
		}
...
if ( ! function_exists( 'xwoo_plugin_fw_gutenberg_get_registered_blocks' ) ) {
	/**
	 * Return an array with the registered blocks
	 *
	 * @return array
	 */
	function xwoo_plugin_fw_gutenberg_get_registered_blocks() {
		return xwoo_plugin_fw_is_gutenberg_enabled() ? XWOO_Gutenberg()->get_registered_blocks() : array();
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_gutenberg_get_to_register_blocks' ) ) {
	/**
	 * Return an array with the blocks to register
	 *
	 * @return array
	 */
	function xwoo_plugin_fw_gutenberg_get_to_register_blocks() {
		return xwoo_plugin_fw_is_gutenberg_enabled() ? XWOO_Gutenberg()->get_to_register_blocks() : array();
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_get_default_logo' ) ) {
	/**
	 * Get the default SVG logo
	 *
	 * @return string default logo image url
	 */
	function xwoo_plugin_fw_get_default_logo() {
		return YIT_CORE_PLUGIN_URL . '/assets/images/xwoo-icon.svg';
	}
}

if ( ! function_exists( 'xwoo_set_wrapper_class' ) ) {
	/**
	 ...
	 */
	function xwoo_set_wrapper_class( $class = '' ) {
		$new_class = 'xwoo-plugin-ui';
...
if ( ! function_exists( 'xwoo_get_date_formats' ) ) {
	/**
	 ...
	 * @since      3.1
	 * @deprecated 3.5 | use xwoo_get_date_formats() instead
	 */
	function xwoo_get_date_format( $js = true ) {
		return xwoo_get_date_formats( $js );
	}
}
...
if ( ! function_exists( 'xwoo_get_date_formats' ) ) {
	 ...
	 * @since  3.5
	 */
	function xwoo_get_date_formats( $js = true ) {
		$date_formats = array(
			'F j, Y' => 'F j, Y',
			'Y-m-d'  => 'Y-m-d',
			'm/d/Y'  => 'm/d/Y',
		...
		return apply_filters( 'xwoo_plugin_fw_date_formats', $date_formats, $js );
	}
}

if ( ! function_exists( 'xwoo_get_time_formats' ) ) {
	/**
	 ...
	 * @since  3.5
	 */
	function xwoo_get_time_formats() {
		...
		return apply_filters( 'xwoo_plugin_fw_time_formats', $time_formats );
	}
}


if ( ! function_exists( 'xwoo_format_toggle_title' ) ) {
	/**
	 ...
	 * @since  3.1
	 */
	function xwoo_format_toggle_title( $title, $values ) {
...
if ( ! function_exists( 'xwoo_plugin_fw_load_update_and_licence_files' ) ) {
	/**
	 * Load premium file for license and update system
	 *
	 * @author Andrea Grillo <andrea.grillo@xwooemes.com>
	 */
	function xwoo_plugin_fw_load_update_and_licence_files() {
		...
		if ( empty( $plugin_upgrade_fw_data ) ) {
			...
			if ( file_exists( $plugin_upgrade_path ) ) {
				$required_files = array(
					$plugin_upgrade_path . '/lib/xwoo-licence.php',
					$plugin_upgrade_path . '/lib/xwoo-plugin-licence.php',
					$plugin_upgrade_path . '/lib/xwoo-theme-licence.php',
					$plugin_upgrade_path . '/lib/xwoo-plugin-upgrade.php',
				);
...
if ( ! function_exists( 'xwoo_plugin_fw_remove_duplicate_classes' ) ) {
	/**
	 ...
	 * @return string
	 * @since  3.2.2
	 * @author Emanuela Castorina <emanuela.castorina@xwooemes.com>
	 */
	function xwoo_plugin_fw_remove_duplicate_classes( $classes ) {
...
if ( ! function_exists( 'xwoo_plugin_fw_add_requirements' ) ) {
	/**
	 ...
	 */
	function xwoo_plugin_fw_add_requirements( $plugin_name, $requirements ) {
		if ( ! empty( $requirements ) ) {
			XWOO_System_Status()->add_requirements( $plugin_name, $requirements );
		}
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_parse_dimensions' ) ) {
	/**
	 ...
	 * @return array
	 */
	function xwoo_plugin_fw_parse_dimensions( $values ) {
		$dimensions = array();
...
if ( ! function_exists( 'xwoo_plugin_fw_get_dimensions_by_option' ) ) {
	/**
	 ...
	 * @return array|bool
	 */
	function xwoo_plugin_fw_get_dimensions_by_option( $option, $default = false ) {
		$dimensions = get_option( $option, false );

		return ! ! $dimensions ? xwoo_plugin_fw_parse_dimensions( $dimensions ) : $default;
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_extract' ) ) {
	/**
	 * Extract array variables
	 *
	 * Usage example:
	 * ```
	 * list ( $type, $class, $value ) = xwoo_plugin_fw_extract( $field, 'type', 'class', 'value' );
	 * ```
	 *
	 ...
	 * @since 3.5
	 */
	function xwoo_plugin_fw_extract( $array, ...$keys ) {
		return array_map(
...
if ( ! function_exists( 'xwoo_plugin_fw_register_elementor_widget' ) ) {
	/**
	 ...
	 * @since 3.6.0
	 */
	function xwoo_plugin_fw_register_elementor_widget( $widget_name, $widget_options ) {
		XWOO_Elementor::instance()->register_widget( $widget_name, $widget_options );
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_register_elementor_widgets' ) ) {
	/**
	 ...
	 * @since 3.6.0
	 */
	function xwoo_plugin_fw_register_elementor_widgets( $widgets, $map_from_gutenberg = false ) {
		foreach ( $widgets as $widget_name => $widget_options ) {
			...
			xwoo_plugin_fw_register_elementor_widget( $widget_name, $widget_options );
		}
	}
}

if ( ! function_exists( 'xwoo_plugin_fw_copy_to_clipboard' ) ) {
	/**
	 ...
	 * @since 3.6.2
	 */
	function xwoo_plugin_fw_copy_to_clipboard( $value, $field = array() ) {
		$defaults      = array(
			'id'    => '',
			'value' => $value,
		);
...
if ( ! function_exists( 'xwoo_plugin_fw_add_utm_data' ) ) {
	/**
	 ...
	 * @since 3.6.10
	 */
	function xwoo_plugin_fw_add_utm_data( $url, $slug, $campaign = 'plugin-version-author-uri', $source = 'wp-dashboard' ) {
