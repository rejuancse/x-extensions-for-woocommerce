...
class Product_Search {
    ...
    function products_search_callback_function() {
        ...
        $output .= '<form class="search_form_shortcode" role="search" action="'.esc_url( home_url( '/' ) ).'" method="get">';
            $output .= '<input class="xewc-ajax-search" data-url="'.esc_url( admin_url( 'admin-ajax.php?action=xewc_product_search' ) ).'" type="text" name="s" value="'.get_search_query().'" placeholder="'.esc_attr__( 'Search products...', 'x-extensions-for-woocommerce' ).'"/>';
            if($btn_off == 'true') { 
